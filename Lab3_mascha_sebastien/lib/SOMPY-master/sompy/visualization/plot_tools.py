...
def plot_hex_map(d_matrix, titles=[], colormap=cm.gray, shape=[1, 1], comp_width=5, hex_shrink=1.0, fig=None,
    ...
    d_matrix = np.flip(np.flip(d_matrix, axis=0), axis=1)
    ...
    def create_grid_coordinates(x, y):
        ...
        #print(np.array(list(reversed(coordinates))))
        #print(np.flip(np.array(list(reversed(coordinates))), axis=1))
        return (np.flip(np.array(list(coordinates)), axis=0), x, y)
        #return (np.flip(np.flip(np.array(coordinates), axis=0), axis=1), x, y)
